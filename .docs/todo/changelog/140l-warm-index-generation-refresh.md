---
id: 140l-warm-index-generation-refresh
...
phase: l
status: done
patch_scope: "Probe search while publishing a new generation in controlled test flow."
blast_radius: high
...
acceptance: "Concurrent read smoke is complete when the declared patch surface exists, public search compatibility is preserved, and the validation command records passing output or a concrete blocker."
exit_criterion: "zig build test"
validation: "zig build test"
expected_exit_code: 0
expected_output_pattern: "Build Summary"
evidence: "C:\\Users\\Savage\\.local\\zig\\zig-x86_64-windows-0.16.0\\zig.exe build test --summary all -> exit 0; Build Summary: 3/3 steps succeeded; 124/124 tests passed. Changed file: src/core/generation.zig. Added .ix/index/current.ixgen stable current manifest publication and a controlled refresh smoke proving an epoch-51 reader pin remains retained after epoch-52 publication."
conflict_surface: ""
invariants:
...
source_message_proof_obligation: "Concurrent read smoke preserves or implements the cited original request within the warm-layer execution program."
entry_state: "Dependency 140k-warm-index-generation-refresh is archived with non-PLACEHOLDER evidence before this unit starts."
...
dependencies: "140k-warm-index-generation-refresh"
next_todo: /todo/pending/140m-warm-index-generation-refresh.md
...
resumption_point: ""
---
# 140l Concurrent read smoke

## Execute Now
Concurrent read smoke: Probe search while publishing a new generation in controlled test flow.
...
|---------------|---------------------------|------------------------|------------------|
| U3 | "Then let's attempt this experiment. Go all out." | Concurrent read smoke must preserve this request fragment. | Validation output plus changed-file review for this unit. |
| U5 | "To slice this entire research into at least 90 to-do slices using the planning spec skill." | Concurrent read smoke must preserve this request fragment. | Validation output plus changed-file review for this unit. |
| U6 | "Once that is established, proceed to implementation." | Concurrent read smoke must preserve this request fragment. | Validation output plus changed-file review for this unit. |
...
## Entry State
- The file `.docs/todo/changelog/140k-warm-index-generation-refresh.md` exists and carries non-PLACEHOLDER evidence.
...
- Concurrent read smoke is complete and the next unit can verify it from filesystem state plus archived evidence.
...
## Next todo
`/todo/pending/140m-warm-index-generation-refresh.md`
...
- [x] Move this file to `/todo/changelog/140l-warm-index-generation-refresh.md` verified.
