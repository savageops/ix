---
id: 140k-warm-index-generation-refresh
...
phase: k
status: done
patch_scope: "Expose generation epoch, refresh status, and fallback reason in JSON stats."
blast_radius: high
...
acceptance: "Telemetry is complete when the declared patch surface exists, public search compatibility is preserved, and the validation command records passing output or a concrete blocker."
exit_criterion: "zig build test"
validation: "zig build test"
expected_exit_code: 0
expected_output_pattern: "Build Summary"
evidence: "C:\\Users\\Savage\\.local\\zig\\zig-x86_64-windows-0.16.0\\zig.exe build test --summary all -> exit 0; Build Summary: 3/3 steps succeeded; 123/123 tests passed. Changed files: src/core/stats.zig, src/cli/output.zig. Added generation_refresh JSON stats with enabled, available, epoch, refresh_status, and fallback_reason defaults."
conflict_surface: ""
invariants:
...
source_message_proof_obligation: "Telemetry preserves or implements the cited original request within the warm-layer execution program."
entry_state: "Dependency 140j-warm-index-generation-refresh is archived with non-PLACEHOLDER evidence before this unit starts."
...
dependencies: "140j-warm-index-generation-refresh"
next_todo: /todo/pending/140l-warm-index-generation-refresh.md
...
## Execute Now
Telemetry: Expose generation epoch, refresh status, and fallback reason in JSON stats.
...
|---------------|---------------------------|------------------------|------------------|
| U3 | "Then let's attempt this experiment. Go all out." | Telemetry must preserve this request fragment. | Validation output plus changed-file review for this unit. |
| U5 | "To slice this entire research into at least 90 to-do slices using the planning spec skill." | Telemetry must preserve this request fragment. | Validation output plus changed-file review for this unit. |
| U6 | "Once that is established, proceed to implementation." | Telemetry must preserve this request fragment. | Validation output plus changed-file review for this unit. |
...
## Entry State
- The file `.docs/todo/changelog/140j-warm-index-generation-refresh.md` exists and carries non-PLACEHOLDER evidence.
...
- Telemetry is complete and the next unit can verify it from filesystem state plus archived evidence.
...
## Next todo
`/todo/pending/140l-warm-index-generation-refresh.md`
...
- [x] Move this file to `/todo/changelog/140k-warm-index-generation-refresh.md` verified.
