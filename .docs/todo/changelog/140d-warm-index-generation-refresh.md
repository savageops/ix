---
id: 140d-warm-index-generation-refresh
...
phase: d
status: done
patch_scope: "Write manifest tmp file and atomic rename/swap into search-visible state."
blast_radius: high
...
acceptance: "Atomic publish is complete when the declared patch surface exists, public search compatibility is preserved, and the validation command records passing output or a concrete blocker."
exit_criterion: "zig build test"
validation: "zig build test"
expected_exit_code: 0
expected_output_pattern: "Build Summary"
evidence: "Validation: `zig build test --summary all` exit 0 -> Build Summary: 3/3 steps succeeded; 116/116 tests passed. Added publishManifestBytes using generation_dir/tmp_dir creation plus createFileAtomic replace, sync, and visible manifest replacement tests; empty manifest body fails closed."
conflict_surface: ""
invariants:
...
source_message_proof_obligation: "Atomic publish preserves or implements the cited original request within the warm-layer execution program."
entry_state: "Dependency 140c-warm-index-generation-refresh is archived with non-PLACEHOLDER evidence before this unit starts."
...
dependencies: "140c-warm-index-generation-refresh"
next_todo: /todo/pending/140e-warm-index-generation-refresh.md
...
## Execute Now
Atomic publish: Write manifest tmp file and atomic rename/swap into search-visible state.
...
|---------------|---------------------------|------------------------|------------------|
| U3 | "Then let's attempt this experiment. Go all out." | Atomic publish must preserve this request fragment. | Validation output plus changed-file review for this unit. |
| U5 | "To slice this entire research into at least 90 to-do slices using the planning spec skill." | Atomic publish must preserve this request fragment. | Validation output plus changed-file review for this unit. |
| U6 | "Once that is established, proceed to implementation." | Atomic publish must preserve this request fragment. | Validation output plus changed-file review for this unit. |
...
## Entry State
- The file `.docs/todo/changelog/140c-warm-index-generation-refresh.md` exists and carries non-PLACEHOLDER evidence.
...
- Atomic publish is complete and the next unit can verify it from filesystem state plus archived evidence.
...
## Next todo
`/todo/pending/140e-warm-index-generation-refresh.md`
...
- [x] Move this file to `/todo/changelog/140d-warm-index-generation-refresh.md` verified.
