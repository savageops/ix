...
The evidence supports a deterministic two-pass extractive pipeline. Pass one streams bytes through UTF-8 validation, source hashing, line/structural-unit detection, fixed-memory lexical frequency evidence, and duplicate-candidate fingerprints. Pass two replays the file, verifies duplicate candidates by exact bytes, computes explainable integer salience and marginal novelty, and emits only complete selected units. Selection is budget-aware before rendering; retained units return to source order. The first and last document units are the absolute reconstruction floor. Obligations, identifiers, headings, paths, commands, hashes, metrics, and errors receive strong transparent score weight and labeled-corpus retention gates rather than an unbounded absolute floor: adversarial documents can contain these features on every line. A parser adapter may improve supported source files, but generic document boundaries remain the baseline and failure fallback.

Three profiles should differ by admissible loss, not adjectives. The conservative profile may remove only verified duplicate units and reducible whitespace; if unique complete units cannot fit, it returns a typed budget failure. The balanced profile may omit low-marginal-value unique units while strongly prioritizing obligations, identifiers, headings, paths, commands, hashes, metrics, errors, and boundary context. The emergency profile raises omission pressure but keeps the same source-boundary floor and reports itself as lossy. No profile rewrites retained bytes.
...
## Implementation falsification result

The native implementation passed the exact-byte, source-coordinate, SHA-256, deterministic-output, UTF-8, binary, long-line, duplicate-collision, source-order, visible-omission, and complete-stdout-budget gates in the focused suite. A review correction made duplicate canonicalization explicitly source-ordered and retained both source boundaries even when their bytes occur elsewhere.

The representative report at `.docs/reports/min-context-compaction-20260716.json` uses exact labeled facts rather than fluent-output judgment. At the measured medium budgets, native recall versus equal-byte prefix recall was: doctrine `0.667 vs 0.833`, source `0.833 vs 0.333`, README `0.833 vs 0.667`, operational ledger `0.833 vs 0.500`, repetitive log `1.000 vs 0.167`, JSONL `1.000 vs 0.200`, benchmark journal `1.000 vs 0.333`, and low-redundancy prose `0.667 vs 0.333`. The negative doctrine result is retained because it defines the boundary: small, front-loaded documents may not benefit.

This proves useful knowledge retention on seven corpus shapes and material improvement on the four-class acceptance floor. It does not prove semantic completeness. `inspect` remains exact source truth, `xo` remains query-guided retrieval, and `min` is accepted only as a query-free bounded projection with explicit loss accounting.
