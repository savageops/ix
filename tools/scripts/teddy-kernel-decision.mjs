...
function candidateMoves(summary, leakSummary, quality, focusedSlowest) {
  ...
  const discoverAttribution = leakSummary?.leakAttribution?.discoverAttribution ?? null;
  const discoverAttributionReason = leakSummary?.nextRepairTarget === "discover" && discoverAttribution != null
    ? ` Discover attribution: filesDiscoveredParity=${discoverAttribution.filesDiscoveredParity}, filesSkippedParity=${discoverAttribution.filesSkippedParity}, effectiveRootsDelta=${discoverAttribution.deltas?.effectiveRoots?.median ?? null}, filesDiscoveredDelta=${discoverAttribution.deltas?.filesDiscovered?.median ?? null}, ignoreFilesLoadedDelta=${discoverAttribution.deltas?.ignoreFilesLoaded?.median ?? null}, discoveryAccessErrorsDelta=${discoverAttribution.deltas?.discoveryAccessErrors?.median ?? null}.`
    : "";
  const moves = [
    {
