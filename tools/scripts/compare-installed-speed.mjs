...
function writePreflightFailureReport({
  ...
  const report = {
    ...
    processMetricCapture: {
      peakRss: "ix_stats_process_memory_with_wrapper_fallback",
      note: "IX speed samples prefer stats.process_memory.peak_resident_bytes and fall back to wrapper metrics when benchmark isolation supplies them.",
    },
...
const installedRepoComparison = {
  ...
  installedPeakWorkingSetBytesMedian: paired.installed.peakWorkingSetBytesSummary?.median ?? null,
  repoPeakWorkingSetBytesMedian: paired.repo.peakWorkingSetBytesSummary?.median ?? null,
  installedPeakWorkingSetBytesMax: paired.installed.peakWorkingSetBytesSummary?.max ?? null,
  repoPeakWorkingSetBytesMax: paired.repo.peakWorkingSetBytesSummary?.max ?? null,
